...
<div class="fieldcontain ${hasErrors(bean: pedido, field: 'fechaEntrega', 'error')} ">
	<label for="fechaEntrega">
		<g:message code="pedido.fechaEntrega.label" default="Fecha Entrega" />
		
	</label>
	<g:datePicker name="fechaEntrega" precision="day"  value="${pedido?.fechaEntrega}" default="none" noSelection="['': '']" />
</div>
...
<div class="fieldcontain ${hasErrors(bean: pedido, field: 'status', 'error')} ">
	...
	<g:select name="status" from="${pedido.constraints.status.inList}" value="${pedido?.status}" valueMessagePrefix="pedido.status" noSelection="['': '']"/>
</div>
